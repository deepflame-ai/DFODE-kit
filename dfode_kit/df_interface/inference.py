...
try:
    ...
    path_r = r"./system/sampleConfigDict"
    with open(path_r, "r") as f:
        data = f.read()
        i = data.index('torchModel_') 
        a = data.index('"',i) 
        b = data.index('"',a+1)
        ...
        i = data.index('frozenTemperature_')
        a = data.index(';', i)
        b = data.rfind(' ',i+1,a)
        frozenTemperature = float(data[b+1:a])

        i = data.index('inferenceDeltaTime_')
        a = data.index(';', i)
        b = data.rfind(' ',i+1,a)
        delta_t = float(data[b+1:a])

        i = data.index('CanteraMechanismFile_')
        a = data.index('"',i) 
        b = data.index('"',a+1)
