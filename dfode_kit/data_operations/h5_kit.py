...
def predict_Y(model, model_path, d_arr, mech, device):
    ...
    next_Y[:, :-1] = (
        next_Y[:, :-1]
        / np.sum(next_Y[:, :-1], axis=1, keepdims=True)
        * (1 - next_Y[:, -1:])
    )
...
def integrate_h5(
    ...
    if cvode_integration:
        ...
        with h5py.File(save_path1, "a") as f:  # Use 'a' to append
            if "cvode_integration" in f:
                del f["cvode_integration"]
            ...
            for dataset_name, processed_data in processed_data_dict.items():
                ...
                print(
                    f"Saved processed dataset: {dataset_name} in cvode_integration group"
                )
    ...
    if nn_integration:
        ...
        if model_settings is None:
            raise ValueError(
                "model_settings must be provided for neural network integration."
            )
        ...
        with h5py.File(save_path2, "a") as f:  # Use 'a' to append
            ...
            for dataset_name, processed_data in processed_data_dict.items():
                ...
                print(
                    f"Saved processed dataset: {dataset_name} in nn_integration group"
                )


def calculate_error(mech_path, save_path1, save_path2, error="RMSE"):
