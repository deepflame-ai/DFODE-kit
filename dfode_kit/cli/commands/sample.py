...
def add_command_parser(subparsers):
    ...
    sample_parser.add_argument('--mech', 
                               required=True,
                               type=str, 
                               help='Path to the mechanism file.')
    sample_parser.add_argument('--case', 
                               required=True,
                               type=str, 
                               help='Root directory containing data.')
    sample_parser.add_argument('--save', 
                               required=True,
                               type=str, 
                               help='Path where the HDF5 file will be saved.')
    sample_parser.add_argument('--include_mesh', 
                               action='store_true', 
...
def handle_command(args):
    ...
    save_arrays_to_hdf5(args.case, args.mech, args.save, include_mesh=args.include_mesh)
    print()
    
    # Optionally load and print the contents of the HDF5 file
    load_and_print_hdf5_contents(args.save)