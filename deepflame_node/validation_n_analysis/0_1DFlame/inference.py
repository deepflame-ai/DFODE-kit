...
device_main = "cuda:2"
device_list = [2] # range(torch.cuda.device_count())
...
try:
    ...
    #load model  
    layers = [n_species +2, 400, 200, 100, 1]
...
def inference(vec0):
    ...
    try:
        with torch.no_grad():
            ...
            input0_normalized[input0_normalized != input0_normalized] = 0

            # input0_normalized[:, -1] = 0 #set Y_AR to 0
            input0_normalized = input0_normalized.to(torch.float16)
